...
export function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 z-50 flex w-[260px] flex-col border-r border-white/8 bg-[#070707]">
      ...
      {/* Brand mark — aligns with header height */}
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-5 space-y-6">
        {TOOLS.map(({ category, items }) => (
          <div key={category}>
            <p className="px-3 mb-2 font-mono text-[9px] uppercase tracking-[0.3em] text-muted-foreground/40">
              {category}
            </p>
                ...
                return (
                  <li key={tool}>
                    <button
                      ...
                      className={cn(
                        "w-full text-left px-3 py-2 rounded-sm font-mono text-xs tracking-wide transition-colors duration-150",
                        "border-l-2",
                        isActive
